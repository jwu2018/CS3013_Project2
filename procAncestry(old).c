...
long get_children(struct task_struct *target_task, ancestry *response);

asmlinkage long (*ref_sys_cs3013_syscall2)(void);
// asmlinkage long (*ref_sys_read)(unsigned int fd, void* buf, size_t count);

asmlinkage long fill_ancestry_struct(unsigned short *target_pid, ancestry *response){
 ...
 */
 long get_children(struct task_struct *target_task, ancestry *response){
  ...
  int i = 0;
 	...
 	// ancestry childAncestry = { childAncestors, childSiblings, childChildren };
...
static int __init interceptor_start(void) {
  ...
  // printk(KERN_INFO "stored original");

  /* Replace the existing system calls */
  disable_page_protection();

  sys_call_table[__NR_cs3013_syscall2] = (unsigned long *)fill_ancestry_struct;
  // printk(KERN_INFO "replaced with fill_ancestry_struct");
